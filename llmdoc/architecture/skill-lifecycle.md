...
- `apps/api/src/services/skill.service.ts` (`createOrUpdateSkill`, `getSkillTag`, `searchSkills`): Core business logic for push, download URL generation, and search. Uses `like` (SQLite).
...
- **7.** `createOrUpdateSkill` upserts `skills` row, uploads tarball to R2 with key `<namespace>/<skillName>/<tag>.tar.gz`, upserts `skill_tags` row.
...
- **2. API:** `GET /api/skills?q=&namespace=&limit=` in `apps/api/src/routes/skills.ts`.
- **3. Backend:** `searchSkills` in `skill.service.ts` uses Drizzle `like` on `skills.name` and `skills.description`, ordered by `downloads` DESC.
