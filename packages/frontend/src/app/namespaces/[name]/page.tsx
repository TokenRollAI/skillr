...
export default async function NamespaceDetailPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  try {
    const nsRes = await fetch(`${BACKEND}/api/namespaces/${nsName}`, { cache: 'no-store' });
    if (nsRes.ok) ns = await nsRes.json();

    const membersRes = await fetch(`${BACKEND}/api/namespaces/${nsName}/members`, { cache: 'no-store' });
    if (membersRes.ok) members = await membersRes.json();

    const skillsRes = await fetch(`${BACKEND}/api/skills?namespace=${nsName}`, { cache: 'no-store' });
    if (skillsRes.ok) nsSkills = await skillsRes.json();
  } catch {}
