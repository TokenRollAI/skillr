...
namespaceRoutes.post('/', requireAuth, async (c) => {
  ...
  try {
    ...
    if (err.message?.includes('UNIQUE constraint failed')) return c.json({ error: 'Namespace already exists' }, 409);
    throw err;
  }
});
...
namespaceRoutes.post('/:name/members', requireAuth, requireNsRole('name', 'maintainer'), async (c) => {
  ...
  try {
    await db.insert(nsMembers).values({ userId, namespaceId: ns.id, role: role || 'viewer', createdAt: new Date().toISOString() });
    return c.json({ success: true }, 201);
  } catch (err: any) {
    if (err.message?.includes('UNIQUE constraint failed')) return c.json({ error: 'User is already a member' }, 409);
    throw err;
  }
});
